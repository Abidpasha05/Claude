...
import { computePricing } from '@restaurant-saas/shared';
...
export async function POST(req: Request) {
  ...
  const { data: restaurant } = await admin
    .from('restaurants')
    .select('id, currency, base_delivery_fee')
    .eq('id', restaurant_id)
    .single();
  if (!restaurant) {
  ...
  const lines = items.map((i: any) => {
    ...
    if (m.status !== 'available') {
      throw new Error(`${m.name} is unavailable`);
    }
    return {
      menu_item_id: m.id,
      name: m.name,
  ...
  const { data: order, error } = await admin
    ...
    .insert({
      ...
      guest_phone,
      order_type,
      status: payment_method === 'cash' ? 'placed' : 'pending_payment',
